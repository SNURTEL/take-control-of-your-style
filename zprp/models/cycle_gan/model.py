...
class CycleGAN(pl.LightningModule):
    ...
    def __init__(
        ...
        Args:
            ...
            lambda_param: Factor to scale the cycle loss by. Defaults to 10.0.
    ...
    def _discriminator_loss(self, real: Tensor, fake: Tensor) -> Tensor:
        ...
        """
        real_loss = self.lsgan_loss(real, torch.ones_like(real))
        fake_loss = self.lsgan_loss(fake, torch.zeros_like(fake))
    ...
    def training_step(self, batch: Tensor, batch_idx: int) -> Tensor | Mapping[str, Any] | None:
        ...
        dx_loss = self._discriminator_loss(real=self.dx(real_x), fake=self.dx(fake_x))
        dy_loss = self._discriminator_loss(real=self.dy(real_y), fake=self.dx(fake_y))
